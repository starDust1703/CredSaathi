...
const Home = () => {
  ...
  // Load initial chat messages if chatId exists
  ...
  const handlePrompt = async () => {
		...
		// show backend reply
		if (data?.data?.reply) {
			setMessages(prev => [...prev, data.data.reply]);
		}
  ...
  return (
    <div className="h-screen w-screen flex flex-col">
      ...
      <main
        className={`mt-14 mb-24 h-full w-full flex flex-col items-center overflow-x-hidden ${
          messages.length > 0 ? "justify-between" : "justify-center"
        }`}
      >
        <div
          className={`flex w-full flex-col items-center justify-center gap-4 ${
            messages.length > 0 ? "p-16" : "mb-5"
          }`}
        >
          {messages.length > 0 ? (
            messages.map((msg, i) =>
        ...
        <div
          className={`w-[102vw] bg-[#00031e81] h-24 fixed -bottom-1 backdrop-blur-xs blur-xs ${
            messages.length === 0 && "hidden"
          }`}
        ></div>

        <div
          className={`w-2/3 ${messages.length > 0 && "fixed"} bottom-6 bg-[#303030] flex justify-between items-center rounded-4xl shadow-lg`}
        >
          <img
            src="plus.svg"
            alt="add"
